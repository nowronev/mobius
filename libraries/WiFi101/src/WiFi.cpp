	...
	switch (u8MsgType) {
		...
		case M2M_WIFI_RESP_PROVISION_INFO:
		{
			...
			if (pstrProvInfo->u8Status == M2M_SUCCESS) {
				...
				memset(WiFi._password, 0, M2M_MAX_SSID_LEN);
				memcpy(WiFi._password, (char *)pstrProvInfo->au8Password, strlen((char *)pstrProvInfo->au8Password));
...
uint8_t WiFiClass::startConnect(const char *ssid, uint8_t u8SecType, const void *pvAuthInfo)
{
	...
	memset(_password, 0, M2M_MAX_SSID_LEN);
	memcpy(_password, (char*) pvAuthInfo, strlen((char*)pvAuthInfo));
	return _status;
}
...
uint8_t WiFiClass::beginProvision2()
{
	return beginProvision2(1);
}

uint8_t WiFiClass::beginProvision2(uint8_t channel)
{
	uint8_t mac[6];
	char provSsid[13];
